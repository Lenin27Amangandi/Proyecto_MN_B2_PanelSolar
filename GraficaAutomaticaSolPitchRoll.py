...
def simulate_sun_motion(date, duration, ax, sun_point, sun_vector, roll_vector, pitch_vector, normal_vector, angle_text, time_text, pitch_roll_text, times):
    ...
    def update_frame(i):
        ...
        # El vector de la normal del panel (perpendicular al sol)
        normal_vector.set_data([0, -sun_x], [0, -sun_y])  # Normal al sol (hacia el panel)
        normal_vector.set_3d_properties([0, -sun_z])
...
# # Función para simular el movimiento del sol y el panel solar
# def simulate_sun_motion(date, duration, ax, sun_point, sun_vector, roll_vector, pitch_vector, angle_text, time_text, pitch_roll_text, times):
#     interval = timedelta(minutes=10)
#     times = [date + interval * i for i in range(int(duration * 60 / 10))]
...
#         return sun_point, sun_vector, roll_vector, pitch_vector, angle_text, time_text, pitch_roll_text
...
#     # Llamar a la función de simulación para actualizar el gráfico
#     ani = FuncAnimation(fig, simulate_sun_motion(start_datetime, simulation_duration, ax, sun_point, sun_vector, roll_vector, pitch_vector, angle_text, time_text, pitch_roll_text, []),
#                         frames=int(simulation_duration * 60 / 10),
#                         interval=500, blit=False)
